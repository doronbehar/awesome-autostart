...
return function (config)
	...
	obj.start = function()
		if gears.filesystem.make_directories(config.pids_path) then
			obj.logger:debug('Creating directories for pid files: ' .. config.pids_path)
		else
			obj.logger:fatal('Couldn\'t create directories for logs, check the permissions and existence of ' .. config.log.dir_path )
			return
		end
