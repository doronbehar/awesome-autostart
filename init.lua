...
local autostart = {}
autostart.init = function(config)
	...
	autostart.logger = Logger(config.log.handler, config.log.settings)
	if gears.filesystem.make_directories(config.pids_path) then
		autostart.logger:debug('Creating directories for pid files: ' .. config.pids_path)
	else
		autostart.logger:fatal('Couldn\'t create directories for logs, check the permissions and existence of ' .. config.log.dir_path )
		return
	end
	for i = 1,#config.programs do
		...
		autostart[config.programs[i]] = prog
	end
end
